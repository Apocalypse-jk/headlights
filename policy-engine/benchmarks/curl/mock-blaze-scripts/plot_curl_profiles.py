...
For example, these files are aggregated into one plot:
    ...
    runs/mock-blaze/opa/run_01/30000patients5warmup30iterations1concurrency/measurements.csv
    runs/mock-blaze/opa/run_02/30000patients5warmup30iterations1concurrency/measurements.csv

Rows are filtered to phase=measurement by default and duration_ms values are
grouped by the Toxiproxy profile column.

Usage:
    python policy-engine/benchmarks/curl/mock-blaze-scripts/plot_curl_profiles.py
    python policy-engine/benchmarks/curl/mock-blaze-scripts/plot_curl_profiles.py --succeeded-only
"""
...
def default_out_dir() -> Path:
    return Path(__file__).resolve().parent.parent / "plots" / "mock-blaze"
